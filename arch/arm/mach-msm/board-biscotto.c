...
static void tsu6721_callback(enum cable_type_t cable_type, int attached)
{
	...
	value.intval <<= ONLINE_TYPE_MAIN_SHIFT;
...
static void muic77693_mhl_cb(bool attached, int charger)
{
	...
	if (attached) {
		...
		} else if (charger == 0x00) {
			pr_info("debug_message : TA charger\n");
			power_type = ONLINE_POWER_TYPE_TA;
		} else if (charger == 0x03) {
			pr_info("debug_message : USB charger\n");
...
static void barcode_emul_poweron(int onoff)
{
	...
	if (barcode_vreg_2p85 == NULL) {
		barcode_vreg_2p85 = regulator_get(NULL, "8917_l9");
