...
#if defined(CONFIG_SENSORS_SSP)
static void ssp_sensor_power_on_vdd(int, int);
#endif
...
#ifdef CONFIG_SENSORS_SSP
#define GPIO_AP_MCU_INT 49
#define GPIO_MCU_AP_INT 10
#define GPIO_MCU_AP_INT_2 11
#define GPIO_MCU_NRST 68
#define GPIO_AP_MCU_SDA_18V 44
#define GPIO_AP_MCU_SCL_18V 45
#define GPIO_MCU_CHG 48
#endif
...
static void ssp_set_prox_light_power(bool onoff);
static void ssp_set_prox_led_power(bool onoff);
...
static struct ssp_platform_data ssp_pdata = {
	...
	.set_prox_light_power = ssp_set_prox_light_power,
	.set_prox_led_power = ssp_set_prox_led_power,
	.sensor_power_on_vdd = ssp_sensor_power_on_vdd,
};
...
static void ssp_sensor_power_on_vdd(int onoff_l9, int onoff_lvs4)
{
	int ret;

	pr_info("[SSP]%s\n", __func__);
	if (vsensor_2p85 == NULL) {
		vsensor_2p85 = regulator_get(NULL, "8917_l9");
	...
	if (vsensor_1p8 == NULL) {
		vsensor_1p8 = regulator_get(NULL, "8917_lvs4");
...
static int initialize_ssp_gpio(void)
{
	...
	pr_info("[SSP]%s\n", __func__);
    gpio_tlmm_config(GPIO_CFG(GPIO_AP_MCU_INT, 0,
    ...
    gpio_tlmm_config(GPIO_CFG(GPIO_MCU_AP_INT, 0,
...
static void ssp_set_prox_light_power(bool onoff)
{
    ...
    if (vsub_sensor_3p0 == NULL) {
        ...
        if (IS_ERR(vsub_sensor_3p0)) {
            pr_err("lp8720 : could not get vsub_sensor_3p0, rc = %ld\n",
                PTR_ERR(vsub_sensor_3p0));
            vsub_sensor_3p0 = NULL;
        }
	if (vsub_sensor_3p0 != NULL){
		ret = regulator_set_voltage(vsub_sensor_3p0, 3000000, 3000000);
		if (ret)
		pr_err("%s: error sub_sensor_3p0 setting voltage ret=%d\n",
			__func__, ret);
	}
    }
    if(vsub_sensor_3p0 != NULL) {
        if(onoff) {
    		ret = regulator_enable(vsub_sensor_3p0);
    		if (ret)
    			pr_err("%s: error enablinig regulator\n", __func__);
        } else {
    		ret = regulator_disable(vsub_sensor_3p0);
    		if (ret)
...
static void ssp_set_prox_led_power(bool onoff)
{
    ...
    if (vsub_sensor_led_3p0 == NULL) {
        ...
        if (IS_ERR(vsub_sensor_led_3p0)) {
            pr_err("%s: error vsub_sensor_led_3p0 regulator regulator_get ret=%d\n",
                __func__, ret);
            return ;
        }
	if (vsub_sensor_led_3p0 != NULL){
		ret = regulator_set_voltage(vsub_sensor_led_3p0, 3000000, 3000000);
		if (ret)
		pr_err("%s: error vsub_sensor_led_3p0 setting voltage ret=%d\n",
			__func__, ret);
	}
    }
    if(vsub_sensor_led_3p0 != NULL) {
        if(onoff) {
    		ret = regulator_enable(vsub_sensor_led_3p0);
    		if (ret)
    			pr_err("%s: error enablinig regulator\n", __func__);
        } else {
    		ret = regulator_disable(vsub_sensor_led_3p0);
    		if (ret)
    			pr_err("%s: error disabling regulator\n", __func__);
        }
    }
...
static void max77693_haptic_power_onoff(int onoff)
{
	...
	if (!reg_l8) {
		reg_l8 = regulator_get(NULL, "8917_l8");
		ret = regulator_set_voltage(reg_l8, 3000000, 3000000);

		if (IS_ERR(reg_l8)) {
			printk(KERN_ERR"could not get 8917_l8, rc = %ld\n",
				PTR_ERR(reg_l8));
			return;
		}
	}
...
#endif
#ifdef CONFIG_LM48560_RCV
static struct lm48560_platform_data  lm48560_pdata = {
	.en = LM48560_SHDN,
	.boost = LM48560_BOOST_EN,
	.insel = LM48560_INSEL2,
	.ontime = LM48560_TWU_15MS,
	.rtime = LM48560_RLT_500MS,
	.atime = LM48560_ATK_0P83MS,
	.outlimit = LM48560_PLEV_25VPP,
	.gain = LM48560_GAIN_06DB_B24DB,

};

static struct i2c_gpio_platform_data lm48560_i2c_gpio_data = {
	.sda_pin               = GPIO_LM48560_SDA,
	.scl_pin                = GPIO_LM48560_SCL,
	.udelay                 = 2,
	.sda_is_open_drain      = 0,
	.scl_is_open_drain      = 0,
	.scl_is_output_only     = 0,
...
static struct platform_device lm48560_i2c_gpio_device = {
	.name                   = "i2c-gpio",
	.id                     = LM48560_RCV_I2C_BUS_ID,
	.dev.platform_data      = &lm48560_i2c_gpio_data,
};

static struct i2c_board_info lm48560_i2c_board_info[] = {

    {         
		I2C_BOARD_INFO("lm48560", 0x6F),

               .platform_data = &lm48560_pdata,
   },
...
LP8720_VREG_INIT(LDO5, 1200000, 3300000, 0);
LP8720_VREG_INIT(BUCK1, 900000, 2300000, 0);
...
#define MPQ_KP_ROW_BASE		PM8917_GPIO_PM_TO_SYS(1)
#define MPQ_KP_COL_BASE		PM8917_GPIO_PM_TO_SYS(11)

static unsigned int mpq_row_gpios[] = {MPQ_KP_ROW_BASE, MPQ_KP_ROW_BASE+1,
				MPQ_KP_ROW_BASE+2, MPQ_KP_ROW_BASE+3, MPQ_KP_ROW_BASE+5};
static unsigned int mpq_col_gpios[] = {MPQ_KP_COL_BASE, MPQ_KP_COL_BASE+2,
				MPQ_KP_COL_BASE+3,MPQ_KP_COL_BASE+5,MPQ_KP_COL_BASE+6};

static const unsigned int ks02_keymap[] = {
/* KEY(row, col, keycode) */
	/* row = scan, col - sense */
	KEY(0, 0, KEY_SEND),
	KEY(0, 1, KEY_1), 
...
static struct matrix_keymap_data ks02_keymap_data = {
	.keymap		= ks02_keymap,
	.keymap_size	= ARRAY_SIZE(ks02_keymap),
};

static struct matrix_keypad_platform_data ks02_keypad_data = {
	.keymap_data		= &ks02_keymap_data,
...
static struct platform_device ks2_keypad_device = {
	.name           = "ks02_3x4_keypad",
	.id             = -1,
	.dev            = {
		.platform_data  = &ks02_keypad_data,
	},
};
#endif
...
static void flip_init_hw(void)
{
	...
	ret = gpio_request(GPIO_HALL_SENSOR_INT, "HALL_SW");
	if (ret)
...
static void fmradio_power(int on)
{
	...
	if (on) {
		...
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
		gpio_request(GPIO_FM_INT, "fmradio_int");
...
static struct i2c_board_info si47xx_board_info[] __initdata = {
	{
...
static struct i2c_gpio_platform_data si47xx_i2c_gpio_data_rev01 = {
	.sda_pin = GPIO_FM_SDA_1_8V_REV01,
	.scl_pin = GPIO_FM_SCL_1_8V_REV01,
};
static struct i2c_gpio_platform_data si47xx_i2c_gpio_data = {
	.sda_pin = GPIO_FM_SDA_1_8V,
	.scl_pin = GPIO_FM_SCL_1_8V,
};

struct platform_device si47xx_i2c_gpio_device_rev01 = {
	.name = "i2c-gpio",
	.id = MSM_SI47XX_I2C_BUS_ID,
	.dev.platform_data = &si47xx_i2c_gpio_data_rev01,
};
struct platform_device si47xx_i2c_gpio_device = {
	.name = "i2c-gpio",
	.id = MSM_SI47XX_I2C_BUS_ID,
	.dev.platform_data = &si47xx_i2c_gpio_data,
};

struct platform_device *fm_radio_device_rev01[] __initdata = {
	&si47xx_i2c_gpio_device_rev01
};
struct platform_device *fm_radio_device[] __initdata = {
	&si47xx_i2c_gpio_device
};

void __init fm_radio_init(void)
{
	unsigned int gpio_fm_rst = GPIO_FM_RST;
	...
	if (system_rev < BOARD_REV02) {
		...
		platform_add_devices(
			fm_radio_device_rev01,
			ARRAY_SIZE(fm_radio_device_rev01));
	} else {
		platform_add_devices(
			fm_radio_device,
			ARRAY_SIZE(fm_radio_device));
	}
	gpio_request(gpio_fm_rst, "fmradio_rst");
	gpio_tlmm_config(GPIO_CFG(gpio_fm_rst, 0, GPIO_CFG_OUTPUT,
...
static struct platform_device *ks02_devices[] __initdata = {
	&msm8960_device_otg,
...
#ifdef CONFIG_LM48560_RCV
	{
		LM48560_RCV_I2C_BUS_ID,
		lm48560_i2c_board_info,
		ARRAY_SIZE(lm48560_i2c_board_info),
	},
#endif
#ifdef CONFIG_INPUT_MPU3050
	{
...
#ifdef CONFIG_FM_SI4705
	{
		MSM_SI47XX_I2C_BUS_ID,
		si47xx_board_info,
		ARRAY_SIZE(si47xx_board_info),
	},
#endif
#ifdef CONFIG_SENSORS_SSP
	{
...
void __init msm8930_ks02_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
...
#endif
	platform_add_devices(ks02_devices, ARRAY_SIZE(ks02_devices));
#ifdef CONFIG_MSM_CAMERA
	msm8930_init_cam();
#endif
	msm8930_init_mmc();
...
#endif
#if defined(CONFIG_KEYBOARD_MATRIX)
	platform_device_register(&ks2_keypad_device);
#endif
