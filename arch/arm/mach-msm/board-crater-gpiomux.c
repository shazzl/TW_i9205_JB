...
static struct msm_gpiomux_config msm8x30_crater_gpio_configs[] __initdata = {
	{
		.gpio	   = 89, //NC
		.settings = {
...
struct crater_init_sleep_table {
...
static struct crater_init_sleep_table crater_init_sleep_table[] = {
	GPIO_TABLE(msm8x30_crater_gpio_configs),
	GPIO_TABLE_NULL,
	GPIO_TABLE_NULL,
	GPIO_TABLE_NULL,
	GPIO_TABLE_NULL,
...
static struct gpiomux_setting bat_chg_suspend_gpio_config = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv  = GPIOMUX_DRV_2MA,
	.pull = GPIOMUX_PULL_NONE,
	.dir = GPIOMUX_IN,
...
static struct msm_gpiomux_config crater_bat_chg_suspend_config[] __initdata = {
	{
		.gpio    = 2,
		.settings = {
			[GPIOMUX_SUSPENDED] = &bat_chg_suspend_gpio_config,
		},
	},
	{
		.gpio    = 47,
...
static void config_crater_gpio_init_sleep_gpio(void)
{
	...
	for (i = 0; i < index; i++) {
		if (crater_init_sleep_table[i].ptr == NULL)
			continue;

		msm_gpiomux_install(crater_init_sleep_table[i].ptr,
			crater_init_sleep_table[i].size);
	}
}
#endif
...
#ifdef CONFIG_MACH_CRATERTD_CHN_3G
	msm_gpiomux_install(crater_bat_chg_suspend_config, 
			ARRAY_SIZE(crater_bat_chg_suspend_config));
	config_crater_gpio_init_sleep_gpio();
#endif
	return 0;
}
