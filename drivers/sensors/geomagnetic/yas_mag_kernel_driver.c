...
/*
static ssize_t magnectic_selftest_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int id, x, y1, y2, dir, sx, sy, ohx, ohy, ohz;
	int err1, err2, err3, err4, err5, err6, err7;
...
static struct device_attribute dev_attr_selftest =
	__ATTR(selftest, S_IRUGO | S_IWUSR | S_IWGRP, geomagnetic_raw_self_test_show, NULL);
/*
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d\n",
			magdata.xyz.v[0]/1000, magdata.xyz.v[1]/1000,
			magdata.xyz.v[2]/1000);
}*/
...
static DEVICE_ATTR(raw_data,  S_IRUGO | S_IWUSR | S_IWGRP,
		geomagnetic_raw_self_test_noise_show, NULL);
static DEVICE_ATTR(vendor, S_IRUGO | S_IWUSR | S_IWGRP, get_vendor_name, NULL);
static DEVICE_ATTR(name, S_IRUGO | S_IWUSR | S_IWGRP, get_chip_name, NULL);
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int symlink_data_created = 0, symlink_raw_created = 0;
#endif
	...
	if (rt) {
		YLOGE(("geomagnetic_probe: sensors_initialize_symlink failed[%s]\n",
			input_data->name));
			goto err;
	}
	symlink_data_created = 1;
#endif
...
#ifdef CONFIG_SENSOR_USE_SYMLINK
	rt =  sensors_initialize_symlink(input_raw);
	if (rt) {
		YLOGE(("geomagnetic_probe: sensors_initialize_symlink failed[%s]\n",
			input_data->name));
			goto err;
	}
	symlink_raw_created = 1;
#endif
...
err:
	if (data != NULL) {
		if (input_raw != NULL) {
#ifdef CONFIG_SENSOR_USE_SYMLINK
			if(symlink_raw_created) {
				sensors_delete_symlink(input_raw);
			}
#endif
		...
		if (input_data != NULL) {
#ifdef CONFIG_SENSOR_USE_SYMLINK
			if(symlink_data_created) {
				sensors_delete_symlink(input_data);
			}
#endif
