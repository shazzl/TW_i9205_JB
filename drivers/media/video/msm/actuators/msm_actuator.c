...
/*	uint16_t yy = 0, xx = 0, zz = 0;*/
...
#if TEST_NTEMP /* already has value from FROM*/
	...
	if (rc < 0)
		return rc;
#else /* added by Randy for PID */
	CDBG("%s: reg table from user side (0x%x, 0x%x)\n",
		__func__, settings[0].reg_addr, settings[0].reg_data);
	rc = msm_camera_i2c_write(&a_ctrl->i2c_client,
		settings[0].reg_addr, settings[0].reg_data,
		MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
		return rc;
#endif
...
#else /* added by Randy for PID */
	CDBG("%s: reg table from user side (0x%x, 0x%x)\n",
		__func__, settings[1].reg_addr, settings[1].reg_data);
	rc = msm_camera_i2c_write(&a_ctrl->i2c_client,
		settings[1].reg_addr, settings[1].reg_data,
		MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
		return rc;
#endif
	...
	struct msm_actuator_move_params_t *move_params)
{
	...
	int16_t code_boundary = 0;
	
	if(a_ctrl->step_position_table == NULL){
		pr_err("%s, a_ctrl->step_position_table is NULL ", __func__);
		return rc;
	}
	curr_lens_pos = a_ctrl->step_position_table[a_ctrl->curr_step_pos];

	/* workaround for actuator fail - range over*/
	CDBG("msm_actuator_hall_effect_move_focus\n");
	if (dest_step_position < 0)
		dest_step_position = 0;
	if (dest_step_position >= a_ctrl->total_steps)
		dest_step_position = a_ctrl->total_steps - 1;
	CDBG("%s dest(%d), total_steps(%d)\n", __func__,
		dest_step_position, a_ctrl->total_steps);

	code_boundary = a_ctrl->step_position_table[dest_step_position];
#else
	...
	if (num_steps == 0)
		return rc;
#if defined(CONFIG_MACH_KS02)
	if (damping_params == NULL) {
		pr_err("%s, __dbg: damping_parm is NULL ", __func__);
		return rc;
	}

	CDBG("msm_actuator_hall_effect_move_focus: new copy_from_user - 1\n");
	if (copy_from_user(&ringing_params[0],
		(void __user *)damping_params,
		(sizeof(struct damping_params_t) * MAX_ACTUATOR_REGION))) {
		rc = -EINVAL;
		return rc;
	}
	CDBG("msm_actuator_hall_effect_move_focus: new copy_from_user - 2\n");
	damping_params = &ringing_params[0];
	CDBG("msm_actuator_hall_effect_move_focus: new copy_from_user - 3\n");
#endif
	damping_code_step = damping_params->damping_step;
	wait_time = damping_params->damping_delay;
#if !defined(CONFIG_MACH_KS02)
...
#if defined(CONFIG_MACH_KS02)
		rc = msm_camera_i2c_write(&a_ctrl->i2c_client,
			0x00, next_lens_pos, MSM_CAMERA_I2C_BYTE_DATA);
		if (rc < 0)
			return rc;
		usleep(wait_time);
#else
...
#if defined(CONFIG_MACH_KS02)
		rc = msm_camera_i2c_write(&a_ctrl->i2c_client,
			0x00, code_boundary, MSM_CAMERA_I2C_BYTE_DATA);
		if (rc < 0)
			return rc;
		usleep(wait_time);
#else
	...
	struct msm_actuator_move_params_t *move_params)
{
	...
	while (a_ctrl->curr_step_pos != dest_step_pos) {
		...
		if ((dest_step_pos * sign_dir) <=
			...
			CDBG("%s: [shchang : ACT] target_lens_pos = %d, sign_dir = %d..\n",
				__func__, target_lens_pos, sign_dir);
			/*shcahng@qaulcomm.com: 1022*/
			...
			CDBG("[shchang : ACT] target_lens_pos = %d, sign_dir = %d..\n",
				target_lens_pos, sign_dir);
			/*shcahng@qaulcomm.com: 1022*/
	...
	struct msm_actuator_set_info_t *set_info)
{
  ...
  int16_t max_dac, min_dac, cur_dac;
  position_pan = start_dac;	  /* 1.2M */
  max_dac = 255;              /* Macro */
  min_dac = 0;                /* Inf */
  inf_offset = -20;
  step_size = 3;
  CDBG("%s: [shchang:AF] position_pan = %d\n", __func__, position_pan);
  code_per_step = a_ctrl->region_params[0].code_per_step;
  CDBG("%s: [shchang:AF] total_steps[%d] code_per_step[%d] ", __func__,
  ...
  if (a_ctrl->step_position_table == NULL)
  	return -EFAULT;
  if(position_pan + inf_offset < min_dac)
  	start_p = min_dac;
  else
  	start_p = position_pan + inf_offset;
  num_steps = set_info->af_tuning_params.total_steps;
  CDBG("[shchang:AF] start_p = %d, num_steps = %d\n", start_p, num_steps);
  a_ctrl->step_position_table[0] = cur_dac = start_p;
  CDBG("[shchang:AF] KAF_Table[ 0] = %d\n", a_ctrl->step_position_table[0]);
  for(i = 1; i < num_steps; i++)
  {
    if( i < 22 )       // calculated for KS02 and Crater
       step_size = 3;
    else
       step_size = 4;    
    cur_dac = cur_dac + step_size;
    if( max_dac > cur_dac )
  	  a_ctrl->step_position_table[i] = cur_dac;
    else
  	  a_ctrl->step_position_table[i] = max_dac;
  	CDBG("[shchang:AF] KAF_Table[%2d] = %d\n", i, a_ctrl->step_position_table[i]);
  }
  return 0;
#else
...
static int32_t msm_actuator_reg_init(struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_set_info_t *set_info) {
	struct reg_settings_t *init_settings = NULL;
	int32_t rc = -EFAULT;
	//uint16_t i = 0;
	...
	if (set_info->actuator_params.init_setting_size) {
		if (a_ctrl->func_tbl->actuator_init_focus) {
			...
			CDBG("%s: PID setttings (0x%x, 0x%x)\n",
				__func__, init_settings->reg_addr,
				init_settings->reg_data);
			rc = a_ctrl->func_tbl->actuator_init_focus(a_ctrl,
				set_info->actuator_params.init_setting_size,
				a_ctrl->i2c_data_type,
				init_settings);
...
#if defined(CONFIG_MACH_KS02)
#if TEST_TEMP /* Randy PID */
	case CFG_ACTUAOTOR_REG_INIT:
		rc = msm_actuator_reg_init(a_ctrl, &cdata.cfg.set_info);
		if (rc < 0)
			pr_err("%s init table failed %d\n", __func__, rc);
		break;
#endif
