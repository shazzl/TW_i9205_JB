...
#define GPIO_FLIP  GPIO_HALL_SENSOR_INT
...
static void set_flip_status(struct sec_flip *flip)
{
	...
	dbg_printk("[FLIP]%s, val:%d, current:%d\n", __FUNCTION__, val, flip_status);
	if (flip_status != val) {
...
static irqreturn_t sec_flip_irq_handler(int irq, void *_flip)
{
	...
	dbg_printk("[FLIP]+%s val:%d,  current:%d\n", __FUNCTION__, gpio_get_value_cansleep(flip->gpio), flip_status);
