...
#include "zinitix_touch_zxt_firmware_ZI03220F.h"
...
#define zinitix_debug_msg(fmt, args...)	\
	if (m_ts_debug_mode)	\
		printk(KERN_ERR "zinitix : [%-18s:%5d]" fmt, \
	...
	u16 reg, u8 *values, u16 length)
{
	...
	zinitix_printk("ts_write_data :ret =%d\n",ret);
	if (ret < 0)
		return ret;
	udelay(DELAY_FOR_POST_TRANSCATION);
	return length;
...
continue_check_point_data:
	...
	if (touch_dev->touch_info.status == 0x0) {
		//nitix_debug_msg("periodical esd repeated int occured\r\n");
		ts_write_cmd(touch_dev->client, ZINITIX_CLEAR_INT_STATUS_CMD);
		udelay(DELAY_FOR_SIGNAL_DELAY);		
		return true;
	}
...
static bool ts_check_need_upgrade(u16 curVersion, u16 curMinorVersion, 
	u16 curRegVersion)
{
	u16	newVersion;
	u16	newMinorVersion;	
...
{
	struct zinitix_touch_dev* touch_dev = (struct zinitix_touch_dev*)data;
	//nitix_debug_msg("threaded irq signalled\r\n");
...
{
	struct zinitix_touch_dev *info = (struct zinitix_touch_dev *)device_data;
	u16 newVersion, newMinorVersion, newRegVersion;
	u16 version;
	char buff[16] = {0};
	...
	version = ((newVersion&0xf)<<12)|((newMinorVersion&0xf)<<8)|(newRegVersion&0xff);

	snprintf(buff, sizeof(buff), "%s%02x%04x", CHIP_INFO, info->cap_info.hw_id, version);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));	
...
static inline void wait_raw_data(struct zinitix_touch_dev *info, int need_skip_cnt)
{
	int skip;
...
//	ret = request_threaded_irq(touch_dev->irq, NULL, ts_int_handler,	IRQF_TRIGGER_LOW | IRQF_ONESHOT, ZINITIX_DRIVER_NAME, touch_dev);
	...
	if (ret) {
		printk(KERN_ERR "unable to register irq.(%s)  : %d\r\n",
			touch_dev->input_dev->name,ret);
