...
#define SAMSUNG_FW_VERSION			0x1000
...
u16 cyttsp4_get_phone_fw_ver(void)
{
    return SAMSUNG_FW_VERSION;
}
EXPORT_SYMBOL(cyttsp4_get_phone_fw_ver);
		...
		struct cyttsp4_touch_firmware *fw)
{
	...
	if (!data->si) {
		dev_dbg(dev,
			"%s: FBL No firmware infomation found, device FW may be corrupted\n",
			__func__);
...
#define SAMSUNG_CAL_SCREEN_TIME         (320 + 160)
#define SAMSUNG_CAL_BUTTON_TIME         (100 + 50)
#define SAMSUNG_CAL_SELF_TIME           (120 + 60)
...
static void cyttsp4_fw_calibrate(struct cyttsp4_device *ttsp)
{
	...
	pm_runtime_get_sync(dev);
 	...
 	rc = cyttsp4_request_exclusive(ttsp, 5000);
	if (rc < 0) {
	...
	for(i=0;i<SAMSUNG_CAL_SCREEN_TIME;i++)
	{
		mdelay(1);
	}
	if (rc < 0) {
	...
	rc = cyttsp4_request_exec_cmd(ttsp, CY_MODE_CAT,
			cmd_buf, 2, return_buf, 1, 5000);
	for(i=0;i<SAMSUNG_CAL_BUTTON_TIME;i++)
	{
		mdelay(1);
	}
	if (rc < 0) {
	...
	rc = cyttsp4_request_exec_cmd(ttsp, CY_MODE_CAT,
			cmd_buf, 2, return_buf, 1, 5000);
	for(i=0;i<SAMSUNG_CAL_SELF_TIME;i++)
	{
		mdelay(1);
	}
	if (rc < 0) {
...
static int upgrade_from_platform_firmware(struct cyttsp4_device *ttsp,
		struct cyttsp4_touch_firmware *fw)
{
	struct device *dev = &ttsp->dev;
	int rc = -ENOSYS;
